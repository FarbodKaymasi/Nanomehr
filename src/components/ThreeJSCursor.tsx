...
export function ThreeJSCursor() {
  ...
  useEffect(() => {
    ...
    const geometry = new SphereGeometry(0.08, 16, 16); // Reduced geometry complexity
    const material = new MeshBasicMaterial({ 
      color: 0x56CBD7,
      transparent: true,
      opacity: 0.7
    });
    const cursorMesh = new Mesh(geometry, material);
    ...
    const animate = () => {
      ...
      if (cursorMeshRef.current) {
        ...
        cursorMeshRef.current.scale.lerp(new Vector3(targetScale, targetScale, targetScale), 0.15);
        ...
        if (frameRef.current % 2 === 0) { // Update color less frequently
          ...
          cursorMeshRef.current.material.color.lerp(new Color(targetColor), 0.1);
        }
      }
